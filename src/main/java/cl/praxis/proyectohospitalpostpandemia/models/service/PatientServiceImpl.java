...
public class PatientServiceImpl implements PatientService {
    ...
    public boolean update(Patient patient) {
        Patient patientUp = findOne(patient.getId());
        patientUp.setRut(patient.getRut());
        patientUp.setName(patient.getName());
        patientUp.setLastName(patient.getLastName());
        patientUp.setAddress(patient.getAddress());
        patientUp.setPhone(patient.getPhone());
        patientUp.setEmail(patient.getEmail());
        patientUp.setBirthDate(patient.getBirthDate());
        patientUp.setMedicalHistory(patient.getMedicalHistory());
        return true;
    }
    ...
    public List<Patient> search(String query) {
        return patientsList.stream()
                .filter(patient -> patient.getRut().contains(query) || patient.getName().contains(query) || patient.getLastName().contains(query) || patient.getAddress().contains(query) || patient.getPhone().contains(query) || patient.getEmail().contains(query) || patient.getBirthDate().contains(query) || patient.getMedicalHistory().contains(query))
